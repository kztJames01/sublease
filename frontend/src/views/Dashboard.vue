<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <h1 class="text-3xl font-display font-bold text-gray-900 mb-8">Dashboard</h1>

    <!-- Stats -->
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <div class="bg-white rounded-xl shadow-sm p-5">
        <p class="text-sm text-gray-500">Active Listings</p>
        <p class="text-2xl font-bold text-gray-900 mt-1">{{ myListings.filter(l => !l.is_sold).length }}</p>
      </div>
      <div class="bg-white rounded-xl shadow-sm p-5">
      ...
      <div class="bg-white rounded-xl shadow-sm p-5">
        ...
        <p class="text-2xl font-bold text-gray-900 mt-1">{{ myItems.length }}</p>
      </div>
      <div class="bg-white rounded-xl shadow-sm p-5">
        <p class="text-sm text-gray-500">Messages</p>
        <p class="text-2xl font-bold text-gray-900 mt-1">{{ conversationCount }}</p>
      </div>
    </div>
    ...
    <div class="mb-10">
      <div class="flex items-center justify-between mb-4">
        ...
        <router-link to="/listings/new" class="text-primary-600 hover:text-primary-700 text-sm font-medium">+ New Listing</router-link>
      </div>
      ...
      <p v-else class="text-gray-400">No listings yet.</p>
    </div>
    ...
    <div>
      <div class="flex items-center justify-between mb-4">
        ...
        <router-link to="/items/new" class="text-primary-600 hover:text-primary-700 text-sm font-medium">+ New Item</router-link>
      </div>
      <div v-if="myItems.length" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="item in myItems" :key="item.id" class="bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          <div class="p-4">
            ...
            <div class="flex gap-2 mt-3">
              <router-link :to="`/items/${item.id}/edit`" class="text-sm text-gray-500 hover:text-primary-600">Edit</router-link>
              <router-link :to="`/items/${item.id}`" class="text-sm text-primary-600 hover:text-primary-700">View</router-link>
            </div>
          </div>
        </div>
      </div>
